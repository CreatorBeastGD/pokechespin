...
import { PBPokemonData } from "@/components/PokemonData";
...
export class PoGoAPI {
    ...
    static getMovePBByID(moveId: string, moveList: any[]) {
        ...
        if (moveId.startsWith("ACID_SPRAY_PLUS")) {
            return {
                moveId: "ACID_SPRAY_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "LIQUIDATION_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "OUTRAGE_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "DRILL_PECK_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "SEED_BOMB_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "MYSTICAL_FIRE_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "SURF_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
            ...
            return {
                moveId: "PSYBEAM_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
                power: Math.floor(170 * megaMultiplier[(parseInt(moveId.slice(-1)) || 1) - 1]),
                durationMs: 3000,
                energyDelta: -100,
            ...
            return {
                moveId: "BRICK_BREAK_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
                power: Math.floor(150 * megaMultiplier[(parseInt(moveId.slice(-1)) || 1) - 1]),
                durationMs: 1500,
                energyDelta: -100,
            ...
            return {
                moveId: "VOLT_TACKLE_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
                power: Math.floor(170 * megaMultiplier[(parseInt(moveId.slice(-1)) || 1) - 1]),
                durationMs: 3500,
                energyDelta: -100,
            ...
            return {
                moveId: "DYNAMIC_PUNCH_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
                power: Math.floor(130 * megaMultiplier[(parseInt(moveId.slice(-1)) || 1) - 1]),
                durationMs: 2500,
                energyDelta: -100,
                type: "POKEMON_TYPE_FIGHTING",
            ...
            return {
                moveId: "ZAP_CANNON_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
                power: Math.floor(160 * megaMultiplier[(parseInt(moveId.slice(-1)) || 1) - 1]),
                durationMs: 3500,
                energyDelta: -100,
                type: "POKEMON_TYPE_ELECTRIC",
            ...
            return {
                moveId: "FUTURE_SIGHT_PLUS_"+(parseInt(moveId.slice(-1)) || 1),
                power: Math.floor(140 * megaMultiplier[(parseInt(moveId.slice(-1)) || 1) - 1]),
                durationMs: 2500,
                energyDelta: -100,
                type: "POKEMON_TYPE_PSYCHIC",
    ...
    static async getDamageAttackDynamax(attackingPokemon: any, defendingPokemon: any, move: any, attackerStats: any, defenderStats: any, bonusAttacker?: any, bonusDefender?: any, raidMode?: any, maxMoveLevel?: any, additionalBonus?: any, shroomBonus?: any, dynamaxCannon: any = false) {
        ...
        return Calculator.calculateDamage(
            ...
            Calculator.getEffectiveDefenseRawCPM(defendingPokemon.stats.baseDefense, defenderStats[2], (raid === "raid-custom-dmax" || raid === "raid-custom-gmax") ? defenderStats[0] : Calculator.getCPM(defenderStats[0])),
    ...
    static getAllDamageValuesFromTargetMove(
        ...
        return multiplierArray.map(mult => Calculator.calculateDamage(
            ...
            Calculator.getEffectiveDefenseRawCPM(defender.stats.baseDefense, defenderStats[2], (raid === "raid-custom-dmax" || raid === "raid-custom-gmax") ? defenderStats[0] : Calculator.getCPM(defenderStats[0])),
            attacker.type == move.type || attacker?.type2 == move.type ? 1.2 : 1,
            effectiveness,
            move.type,
    ...
    static getDynamaxDelayAfterMove (raidMode: any, defenderId?: any) {
        if (raidMode === "raid-t5-dmax" || raidMode === "raid-custom-dmax") {
            return [0, 500];
        } else {
            return [0, 0];
        }
    ...
    static getEnrageMultiplier(raidMode: any, isEnraged: any) {
        if (!isEnraged) return 1;
        switch (raidMode) {
            case "raid-t5-dmax":
                return 3;
            case "raid-t6-gmax":
            ...
            case "raid-t1-dmax":
                return 3;
            case "raid-custom-dmax":
                return 3;
            case "raid-custom-gmax":
                return 6;
            default:
                return 1;
        }
    ...
    static getDamageMultiplier(raidMode: any, defender?: any) {
        let actualDefender = defender;
        if (raidMode === "raid-custom-dmax") {
            return 1;
        } if (raidMode === "raid-custom-gmax") {
            return 1;
        }
        if (raidMode === "raid-t5-dmax") {
          return 2;
    ...
    static getBestChargedMove(pokemon: any, boss: any, types: any, raidMode: string, customBossHP: number = 1, allMoves?: any, customEGM: number = 4, defenderStats: any = [40,15,15,15], fastMove?: any, level: number = 40, attack: number = 15): {move: any, ept: number, needsMushroom: boolean} {
        let bestMove = null;
        let bestEPT = 0;
        let bestEPTShroom = 0;
        const movesToCheck = pokemon.cinematicMoves.filter((move: any) => {
            return move != "RETURN" && move != "FRUSTRATION" && move != "AEROBLAST_PLUS_PLUS" && move != "SACRED_FIRE_PLUS_PLUS";
        });
        movesToCheck.forEach((move: any) => {
            let moveData = this.getMovePBByID(move, allMoves);
            const damage = this.getDamage(pokemon, boss, moveData, types, [level,attack,15,15], defenderStats, ["EXTREME", false, false, 0], ["EXTREME", false, false, 0], raidMode, 0, 1);
            ...
            const fastMoveUsesForCharged = fastMove ? Math.ceil(100 / fastMove.energyDelta) : 0;
            
            const ept = (fastMoveUsesForCharged + (energyGain * (100 / -moveData.energyDelta))) / ((moveData.durationMs * (100 / -moveData.energyDelta) + fastMoveUsesForCharged * 500) / 500);
        ...
        allMaxPoke.forEach((defender: string) => {
            ...
            bossList.forEach((boss: any) => {  
                ...
                for (let i = 0; i < bossMoves.length; i++) {
                    ...
                    percentAfterLarge = (this.getDamage(bossData, pokemonData, move, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, this.getDamageMultiplier(raidMode, boss)) + (percentAfterLarge*i)) / (i+1);
                    ...
                    percentAfterTargetBestCase = (this.getDamage(bossData, pokemonData, move, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.29 * this.getDamageMultiplier(raidMode, boss)) + (percentAfterTargetBestCase*i)) / (i+1);
                    ...
                    percentAfterTargetWorstCase = (this.getDamage(bossData, pokemonData, move, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.6 * this.getDamageMultiplier(raidMode, boss)) + (percentAfterTargetWorstCase*i)) / (i+1);
                }
    ...
    static getFastestQuickMove(pokemon: any, boss: any, types: any, raidMode: string, allMoves?: any, defenderStats: any = [40,15,15,15], customBossHP: number = 1, customEGM: number = 4) {
        let bestMove = null;
        ...
        pokemon.quickMoves.forEach((move: any) => {
            ...
            const damage = this.getDamage(pokemon, boss, moveData, types, [40,15,15,15], defenderStats, ["EXTREME", false, false, 0], ["EXTREME", false, false, 0], raidMode, 0, 1);
            const energyGain = Calculator.getMaxEnergyGain(damage, raidMode.startsWith("raid-custom") ? customBossHP : Calculator.getEffectiveDMAXHP(raidMode, boss.pokemonId, this.hasDoubleWeaknesses(boss.type, boss.type2, types)), raidMode, boss.pokemonId, customEGM);
            const ept = energyGain / (moveData.durationMs / 500);
            if (ept > bestEPT) {
                bestMove = moveData;
                ...
                if (moveData.durationMs < bestDuration || damage > bestDamage) {
                    bestMove = moveData;
                    bestDuration = moveData.durationMs;
                    bestDamage = damage;
                }
    ...
    static getGeneralBestDefendersDynamax(
        ...
        let graphic: { pokemon: any; large:number; targetBest:number; targetWorst:number; targetAvg: number; tankScore: number; fastMove: any; chargedMove: any | null; pbID?: number}[] = [];
        ...
        if (localStorage.getItem("usePokeboxOnRankings") === "true") {
            ...
            if (pokeboxID) {
                ...
                myPokemon.forEach((pokemon: PBPokemonData) => {
                    ...
                    for (let i = 0; i < bossMoves.length; i++) {
                        const move = bossMoves[i];
                        percentAfterLarge = (this.getDamage(boss, pokemonData, move, types, defenderStatModified, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode,boss.pokemonId), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax")) + (percentAfterLarge*i)) / (i+1);
                        percentAfterTargetBestCase = (this.getDamage(boss, pokemonData, move, types, defenderStatModified, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.29 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode,boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax")) + (percentAfterTargetBestCase*i)) / (i+1);
                        percentAfterTargetWorstCase = (this.getDamage(boss, pokemonData, move, types, defenderStatModified, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.6 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode,boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax")) + (percentAfterTargetWorstCase*i)) / (i+1);
                    }
                    
                    const tankScore = ((Math.max(0, percentAfterLarge + (Math.max(0,(percentAfterTargetBestCase + percentAfterTargetWorstCase)))/2)  / 2))
                    const fastMove = this.getMovePBByID(pokemon.fastAttackId, allMoves);
                    graphic.push({pokemon: pokemonData, large: percentAfterLarge, targetBest: Math.max(0, percentAfterTargetBestCase), targetWorst: Math.max(0, percentAfterTargetWorstCase), targetAvg: ((Math.max(0, (percentAfterTargetBestCase + percentAfterTargetWorstCase)))/2)  ,tankScore: tankScore, fastMove: fastMove, chargedMove: this.getBestChargedMove(pokemonData, boss, types, raidMode, customBossHP, allMoves, customEGM, defenderStatModified, fastMove, pokemon.stats.level, pokemon.stats.atk), pbID: pokemon.id})
                });
        ...
        allMaxPoke.forEach((defender: string) => {
            ...
            for (let i = 0; i < bossMoves.length; i++) {
                ...
                //percentAfterLarge = (Math.max(0, ((Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0])
                //    - Math.max(0, (this.getDamage(boss, pokemonData, move, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, this.getDamageMultiplier(raidMode,false, false, boss)))))) / Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0])) + (percentAfterLarge*i)) / (i+1);
                percentAfterLarge = (this.getDamage(boss, pokemonData, move, types, defenderStatModified, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax")) + (percentAfterLarge*i)) / (i+1);
                //percentAfterTargetBestCase = (((((Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0])
                //    - Math.max(0, (this.getDamage(boss, pokemonData, move, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.4 * this.getDamageMultiplier(raidMode,false, false, boss)))))) / Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0]))) + (percentAfterTargetBestCase*i)) / (i+1);
                percentAfterTargetBestCase = (this.getDamage(boss, pokemonData, move, types, defenderStatModified, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.29 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax")) + (percentAfterTargetBestCase*i)) / (i+1);
                //percentAfterTargetWorstCase = (((((Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0])
                //    - Math.max(0, (this.getDamage(boss, pokemonData, move, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.7 * this.getDamageMultiplier(raidMode,false, false, boss)))))) / Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0]))) + (percentAfterTargetWorstCase*i)) / (i+1);
                percentAfterTargetWorstCase = (this.getDamage(boss, pokemonData, move, types, defenderStatModified, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.6 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax")) + (percentAfterTargetWorstCase*i)) / (i+1);
            }
    ...
    static getBestDefendersDynamax(
        ...
        if (localStorage.getItem("usePokeboxOnRankings") === "true") {
            ...
            if (pokeboxID) {
                ...
                myPokemon.forEach((pokemon: PBPokemonData) => {
                    ...
                    const percentAfterLarge = 
                        this.getDamage(boss, pokemonData, bossLargeAttackData, types, defenderStatModified, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax"))
                    
                    const percentAfterTargetBestCase = 
                        this.getDamage(boss, pokemonData, bossTargetAttackData, types, defenderStatModified, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.3 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax"))

                    const percentAfterTargetWorstCase = 
                        this.getDamage(boss, pokemonData, bossTargetAttackData, types, defenderStatModified, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.6 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax"))
        ...
        allMaxPoke.forEach((defender: string) => {
            ...
            //    - Math.max(0, (this.getDamage(boss, pokemonData, bossLargeAttackData, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, this.getDamageMultiplier(raidMode,false, false, boss)))))) / Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0]))
            const percentAfterLarge = 
                this.getDamage(boss, pokemonData, bossLargeAttackData, types, defenderStatModified, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax"))

            //const percentAfterTargetBestCase = (((Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0])
            //    - Math.max(0, (this.getDamage(boss, pokemonData, bossTargetAttackData, types, defenderStat, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.4 * this.getDamageMultiplier(raidMode,false, false, boss)))))) / Calculator.getEffectiveStamina(pokemonData.stats.baseStamina, attackerStat[3], attackerStat[0]))
            const percentAfterTargetBestCase = 
                this.getDamage(boss, pokemonData, bossTargetAttackData, types, defenderStatModified, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.3 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax"))
            ...
            const percentAfterTargetWorstCase = 
                this.getDamage(boss, pokemonData, bossTargetAttackData, types, defenderStatModified, attackerStat, [weather, false, false, 0], [weather, false, false, 0], "normal", 0, 2 * 0.6 * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customAtkMult : this.getDamageMultiplier(raidMode, boss.pokemonId)), (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax"))
            const tankScore = ((Math.max(0, percentAfterLarge + (Math.max(0,(percentAfterTargetBestCase + percentAfterTargetWorstCase)))/2)  / 2))
            const fastMove = this.getFastestQuickMove(pokemonData, boss, types, raidMode, allMoves, defenderStatModified, customBossHP, customEGM);
            //console.log("Pokemon: " + pokemonData.pokemonId + " Tank Score: " + tankScore);
            graphic.push({pokemon: pokemonData, large: percentAfterLarge, targetBest: Math.max(0, percentAfterTargetBestCase), targetWorst: Math.max(0, percentAfterTargetWorstCase), targetAvg: ((Math.max(0, (percentAfterTargetBestCase + percentAfterTargetWorstCase)))/2)  ,tankScore: tankScore, fastMove: fastMove, chargedMove: this.getBestChargedMove(pokemonData, boss, types, raidMode, customBossHP, allMoves, customEGM, defenderStatModified, fastMove, level)});
    ...
    static GetBestAttackersDynamax(
        ...
        let attackersStat: { pokemon: any; quickMove: any; maxMove: any; damage: number; fastMove: any, chargedMove: any | null; pbID?: number }[] = [];
        ...
        if (localStorage.getItem("usePokeboxOnRankings") === "true") {
            ...
            if (pokeboxID) {
                ...
                myPokemon.forEach((pokemon: PBPokemonData) => {
                    const pokemonData = this.getPokemonPBByID(pokemon.pokemonId, pokemonList)[0];
                    const quickMove = this.getMovePBByID(pokemon.fastAttackId, allMoves);
                    const chargedMove: any = this.getBestChargedMove(pokemonData, boss, types, raidMode, customBossHP, allMoves, customEGM, modDefenderStats, quickMove, pokemon.stats.level, pokemon.stats.atk);
                    const maxMove = this.getDynamaxAttack(pokemonData.pokemonId, quickMove.type, allMoves, pokemon.max.attack, quickMove);
                    if (isDCannon && (pokemon.pokemonId !== "ZACIAN_CROWNED_SWORD_FORM" && pokemon.pokemonId !== "ZAMAZENTA_CROWNED_SHIELD_FORM")) {
                        if (pokemon.max.attack === 1) maxMove.power = maxMove.power === 250 ? 300 : 400;
                        if (pokemon.max.attack === 2) maxMove.power = maxMove.power === 300 ? 350 : 450;
                        if (pokemon.max.attack === 3) maxMove.power = maxMove.power === 350 ? 450 : 550;
                    }
                    //console.log(weather)
                    const damageDone = this.getDamage(pokemonData, boss, maxMove, types, [pokemon.stats.level, pokemon.stats.atk, pokemon.stats.def, pokemon.stats.hp], modDefenderStats, [weather, false, false, 0], [weather, false, false, 0], raidMode, this.getDefenseMultiplier(raidMode), 1);
                    if (isDCannon && (pokemon.pokemonId !== "ZACIAN_CROWNED_SWORD_FORM" && pokemon.pokemonId !== "ZAMAZENTA_CROWNED_SHIELD_FORM")) {
                        if (pokemon.max.attack === 1) maxMove.power = maxMove.power === 300 ? 250 : 300;
                        if (pokemon.max.attack === 2) maxMove.power = maxMove.power === 350 ? 300 : 350;
                        if (pokemon.max.attack === 3) maxMove.power = maxMove.power === 450 ? 350 : 450;
                    }
                    attackersStat.push({pokemon: pokemonData, quickMove: quickMove, maxMove: maxMove, damage: damageDone, fastMove: this.getBestQuickMove(pokemonData, boss, types, raidMode, allMoves), chargedMove: chargedMove, pbID: pokemon.id});
                })
        ...
        allMaxPoke.forEach((attacker: string) => {
            ...
            attackersStat.push({pokemon: pokemonData, quickMove: quickMove, maxMove: maxMove, damage: damageDone, fastMove: this.getBestQuickMove(pokemonData, boss, types, raidMode, allMoves), chargedMove: chargedMove});
        });
        return attackersStat.sort((a, b) => b.damage - a.damage);
    }
    ...
    static convertedPokeboxData: PBPokemonData[] = [];
    ...
    static FromID(pokeboxList: PBPokemonData[], pbID: number) {
        let pokemonToReturn: PBPokemonData | null = null;
        pokeboxList.forEach((pokemon: PBPokemonData) => {
            const result = (pokemon.id).toString() == pbID.toString();
            if (result) {
                pokemonToReturn = pokemon;
            }
        })
        return pokemonToReturn || pokeboxList[0];
    }
    ...
    static async AdvancedSimulationDynamax(
        ...
        customEnergyGainMult: number = 1
    ) {
        let defenderStats = this.convertStats([40,15,15,15], raidMode, defender.pokemonId);
        
        const isCustomDmax = raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax";
        ...
        */
        ...
        let defenderHealth = (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customBossHP : Calculator.getEffectiveStaminaForRaid(defender.stats.baseStamina, defender.stats.raidCP, defender.stats.raidBossCP, raidMode, defender.pokemonId, hasWeakness);
        ...
        for (let i = 0 ; i < attackers.length ; i++) {
            for (let j = 0 ; j < 3 ; j++) {
                ...
                tankScore[i][j] = ((
                    ...
                    - Math.max(0, - shieldHP[i][j] + (this.getDamage(defender, attackers[i][j], defenderLargeAttack,  types, defenderStats, attackersStats[i][j], [weather, false, false, 0], [weather, false, false, friendship[i]], "normal", 0, (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax" ? customBossAtkMult : this.getDamageMultiplier(raidMode,defender.pokemonId)), isCustomDmax))))) / Calculator.getEffectiveStamina(attackers[i][j].stats.baseStamina, attackersStats[i][j][3], attackersStats[i][j][0])
                    + ((Calculator.getEffectiveStamina(attackers[i][j].stats.baseStamina, attackersStats[i][j][3], attackersStats[i][j][0])
        ...
        const dynamaxDelays = this.getDynamaxRaidDelays(raidMode, defender.pokemonId);
        ...
        while (this.sumAllElements(attackerDamage) <= defenderHealth) {            
            ...
            for (let i = 0 ; i < attackers.length ; i++) {
                ...
                if (defenderDamageStart != -1 && !attackerEvades[i] && defenderMove != null && time < defenderDamageStart + defenderMove.damageWindowStartMs && activePokemon[i] < 3 && target === i && targeted && !firstDmgReduction[i]) {
                    const projectedDamageDefender = this.getDamage(defender, attackers[i][activePokemon[i]], defenderMove, types, defenderStats, attackersStats[i][activePokemon[i]], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customBossAtkMult : this.getDamageMultiplier(raidMode,defender.pokemonId)), isCustomDmax);
                ...
                if (!firstDmgReduction[i] && time > 0) {
                    if (attackerDamageStart[i] == -1 && activePokemon[i] < 3 ) {
                        ...
                        const maxEnergyQuickAttack = Calculator.getMaxEnergyGain(projectedDamageQuick, defenderHealth, raidMode, defender.pokemonId, customEnergyGainMult);
                        ...
                        const maxEnergyCinematicAttack = Calculator.getMaxEnergyGain(projectedDamageCinematic, defenderHealth, raidMode, defender.pokemonId, customEnergyGainMult);
                ...
                if (attackerMove[i] !== null && attackerDamageStart[i] > -1 && time === attackerDamageStart[i] + attackerMove[i].damageWindowStartMs && activePokemon[i] < 3) {
                    ...
                    maxEnergy += Calculator.getMaxEnergyGain(projectedDamage, defenderHealth, raidMode, defender.pokemonId, customEnergyGainMult);
                    if (maxEnergy > 100) {
                        maxEnergy = 100;
                    }
                    maxEnergyGain += maxEnergy;
            ...
            if (maxEnergy >= 100) {
                ...
                for (let i = 0 ; i < attackers.length; i++) {
                    ...
                    for (let j = 0 ; j < 3 ; j++) {
                        tankScore[i][j] = (
                            ((Calculator.getEffectiveStamina(attackers[i][j].stats.baseStamina, attackersStats[i][j][3], attackersStats[i][j][0])
                            - Math.max(0, - shieldHP[i][j] + (this.getDamage(defender, attackers[i][j], defenderLargeAttack, types, defenderStats, attackersStats[i][j], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, ((raidMode === "raid-custom-dmax" ? customBossAtkMult : this.getDamageMultiplier(raidMode, defender.pokemonId)) * (advEffects[i] === "bash" ? (1/ Calculator.BashBoost(raidMode)) : 1)), isCustomDmax))))) / Calculator.getEffectiveStamina(attackers[i][j].stats.baseStamina, attackersStats[i][j][3], attackersStats[i][j][0])
                            + ((Calculator.getEffectiveStamina(attackers[i][j].stats.baseStamina, attackersStats[i][j][3], attackersStats[i][j][0])
                            - Math.max(0, - shieldHP[i][j] + (this.getDamage(defender, attackers[i][j], defenderTargetAttack, types, defenderStats, attackersStats[i][j], [weather, false, false, 0], [weather, false, false, 0], "normal", 0, ((raidMode === "raid-custom-dmax" ? customBossAtkMult : this.getDamageMultiplier(raidMode, defender.pokemonId)) * (advEffects[i] === "bash" ? (1/ Calculator.BashBoost(raidMode)) : 1)), isCustomDmax))))) / Calculator.getEffectiveStamina(attackers[i][j].stats.baseStamina, attackersStats[i][j][3], attackersStats[i][j][0])
                          ) / 2
                    }
                ...
                if (targeted) {
                    if (activePokemon[target] < 3) {
                        const projectedDamageDefender = this.getDamage(
                            ...
                            [weather, false, false, 0], 
                            "normal", 
                            this.getEnrageMultiplier(raidMode, desperate), 
                            (attackerEvades[target] ? 1 : 2) * ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? ((advEffects[target] === "bash" ? (1/ Calculator.BashBoost(raidMode)) : 1 ) * customBossAtkMult * this.getDamageMultiplier(raidMode, defender.pokemonId)) : this.getDamageMultiplier(raidMode, defender.pokemonId)) * (advEffects[target] === "bash" ? (1/ Calculator.BashBoost(raidMode)) : 1),
                            isCustomDmax
                        );
                    ...
                    for (let i = 0 ; i < attackers.length ; i++) {
                        if (activePokemon[i] < 3) {
                            const projectedDamageDefender = this.getDamage(
                                ...
                                "normal", 
                                this.getEnrageMultiplier(raidMode, desperate),
                                ((raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? ((advEffects[target] === "bash" ? (1/ Calculator.BashBoost(raidMode)) : 1 ) * customBossAtkMult * this.getDamageMultiplier(raidMode, defender.pokemonId)) : this.getDamageMultiplier(raidMode, defender.pokemonId  )) * (advEffects[i] === "bash" ? (1/ Calculator.BashBoost(raidMode)) : 1),
                                isCustomDmax
                            );
                            const finalDamage = Math.floor(projectedDamageDefender);
                            let finalDamageReduced = finalDamage;
    ...
    static TurnBasedSimulatorAllyTurn(
        ...
        if (gamestatus.allyPokemonMaxHealth.length === 0) {
            ...
            gamestatus.enemyPokemonMaxHealth = (raidMode === "raid-custom-dmax" || raidMode === "raid-custom-gmax") ? customBossHP : Calculator.getEffectiveStaminaForRaid(defender.stats.baseStamina, defender.stats.raidCP, defender.stats.raidBossCP, raidMode, defender.pokemonId, this.hasDoubleWeaknesses(defender.type, defender.type2, types));
        }
        ...
        if (gamestatus.allyCooldown == 0) {
            switch (order) {
                ...
                case "switch0":
                    gamestatus.allyActiveMove = null;
                    gamestatus.allyCooldown = (gamestatus.maxPhaseCounter == 4) ? 0 : localStorage.getItem("slowerSwaps") === "true" ? 1 : 0.5
        ...
        if (gamestatus.allyCooldown > 0) {
            ...
            if (gamestatus.allyCooldown <= 0) {
                ...
                if (gamestatus.allyActiveMove != null) {
                    ...
                    gamestatus.enemyPokemonDamage += projectedDamage;
                    gamestatus.maxEnergy += Calculator.getMaxEnergyGain(projectedDamage, defenderHealth, raidMode, defender.pokemonId, customEnergyGainMult);
                    if (gamestatus.maxEnergy >= 100) {
                        gamestatus.maxEnergy = 100;
                        gamestatus.maxPhaseCounter = 4;
                    }
        ...
        // Custom DMAX
            if (raidMode === "raid-custom-dmax") {
                ...
                // Defender is getting desperate
                if (!gamestatus.enrage && (gamestatus.timer >= 270)) {
                    gamestatus.enrageCurrentMessage = {
                ...
                // Defender' attacks are getting stronger
                if ((gamestatus.bossCastedMove) && (gamestatus.timer >= 300)) {
                    gamestatus.enrageCurrentMessage = {
                ...
                // Defender is getting desperate
                if (!gamestatus.enrage && (gamestatus.timer >= 190)) {
                    gamestatus.enrageCurrentMessage = {
                        message: "The Max Battle Boss is getting desperate!",
                        duration: 0,
                        color: "#fa8585"
                ...
                // Defender' attacks are getting stronger
                if ((gamestatus.bossCastedMove) && (gamestatus.timer >= 220)) {
                    gamestatus.enrageCurrentMessage = {
                        message: "The Max Battle Boss' attacks are getting stronger!",
                        duration: 0,
                        color: "#fa8585"
    ...
    static TurnBasedSimulatorEnemyTurn(
        ...
        let hasWeakness = this.hasDoubleWeaknesses(defender.type, defender.type2, types);
        let defenderHealth = isCustomDmax ? customBossHP : Calculator.getEffectiveStaminaForRaid(defender.stats.baseStamina, defender.stats.raidCP, defender.stats.raidBossCP, raidMode, defender.pokemonId, hasWeakness);
            ...
            if (gamestatus.enemyCooldown <= 0) {
                ...
                if (gamestatus.enemyPrepPhase === true) {
                    ...
                    let projectedDamage = Math.floor(this.getDamage(
                        ...
                        [weather, false, false, 0] , 
                        "normal", 
                        this.getEnrageMultiplier(raidMode, gamestatus.enrage),
                        (gamestatus.damageReduction * (gamestatus.enemyActiveMove?.isTarget ? 2 : 1)) * this.getDamageMultiplier(raidMode, defender.pokemonId) * ((raidMode == "raid-custom-dmax" || raidMode == "raid-custom-gmax") ? customBossAtkMult : 1) * (advEffects === "bash" ? 1/Calculator.BashBoost(raidMode) : 1),
                        isCustomDmax
                    ));
                    let proDamageReal = projectedDamage;
