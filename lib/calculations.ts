...
export class Calculator {
    ...
    static UpcomingGMaxPokemon = [
      ...
      "URSHIFU_SINGLE_STRIKE_GIGANTAMAX",
    ...
    private static CPM_VALUES: { [level: number]: number } = {
        ...
        51: 0.8453,
        51.5: (Math.sqrt((Math.pow(0.8453, 2) + Math.pow(0.8503, 2)) / 2)),
        52: 0.8503,
        52.5: (Math.sqrt((Math.pow(0.8503, 2) + Math.pow(0.8553, 2)) / 2)),
        53: 0.8553,
        53.5: (Math.sqrt((Math.pow(0.8553, 2) + Math.pow(0.8603, 2)) / 2)),
        54: 0.8603,
        54.5: (Math.sqrt((Math.pow(0.8603, 2) + Math.pow(0.8653, 2)) / 2)),
        55: 0.8653,
        55.5: 0.8653,
        56: 0.8653,
        56.5: 0.8653,
      ...
      static getMaxEnergyGain(damage: number, bossHP: number, raidMode: string = "normal", bossId: string = "", customEGM: number = 1) {
        

        let multiplier = raidMode === "normal" ? 1 : raidMode === "raid-t5-dmax" ? 4 : raidMode === "raid-t6-gmax" ? 15 : 1;
        let decimals = raidMode === "normal" ? 0 : raidMode === "raid-t5-dmax" ? 2 : raidMode === "raid-t6-gmax" ? 2 : 0;
        
        multiplier = raidMode.startsWith("raid-custom") ? customEGM : multiplier;
        decimals = raidMode.startsWith("raid-custom") ? 2 : decimals;

        //console.log(damage + " " + bossHP + " " + raidMode + " " + bossId + " " + multiplier);

        let max_energy_gain = damage / bossHP * 200 * multiplier;
      ...
      static getEffectiveDMAXHP(raidMode: string, pokemonId: string, hasWeakness?: boolean) {
        ...
        const t6gmaxHP: Record<string, number> = {
            ...
            CHARIZARD_GIGANTAMAX: 70000,
            BLASTOISE_GIGANTAMAX: 75000,
            GENGAR_GIGANTAMAX: 70000,
            TOXTRICITY_GIGANTAMAX: 100000,
            TOXTRICITY_LOW_KEY_GIGANTAMAX: 100000,
            TOXTRICITY_AMPED_GIGANTAMAX: 100000,
            LAPRAS_GIGANTAMAX: 80000,
            KINGLER_GIGANTAMAX: 100000,
            SNORLAX_GIGANTAMAX: 135000,
